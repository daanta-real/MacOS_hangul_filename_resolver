...
/* [0] 본 프로그램을 만든 목적
    맥에서 한글로 된 파일명을 취급할 때에는 NFD(조합형)이라는 인코딩을 쓴다.
    이는 자모를 초성/중성/종성으로 분리하여 저장하는 인코딩인데,
    '각'이라는 문자를 저장한다면 'ㄱㅏㄱ' 이렇게 뜯어서 저장하는 방식이다.
    맥에서는 이 문자열을 불러올 때 자모를 다시 조합하여 보여주므로 '각'이라고 정상 출력된다.
    
    그러나 윈도에서는 기본적으로 NFC(완성형)만 쓰고 있다.
    NFD(조합형) 인코딩을 지원하지 않으므로, 맥 파일명을 불러오면 'ㄱㅏㄱ' 이렇게 뜯어져 보이게 된다.
    맥에서 저장된 파일 관련 정보를 불러올 때에는 파일명의 인코딩을 NFD(조합형)에서 NFC(완성형)으로
    변환(이것을 정규화라고 한다)시켜 주어야 하는 것이다.

    그것을 위해 필요한 정규화 함수가 String.prototype.normalize()이다.
    이 함수는 정돈되지 않은 문자열을 유니코드 형식에 맞게 정규화시켜주는 함수인데,
    모든 모던 브라우저에서 지원한다.
    하지만 익스플로러에서는 이 함수를 지원하지 않기에,
*/
